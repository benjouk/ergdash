...
// Labels only — the payload shapes, and the documentation for them, are served
// by the server so they cannot drift from what it actually sends.
const WEBHOOK_FORMAT_OPTIONS = [
  { value: 'json', label: 'ErgDash JSON' },
...
function WebhookDocs({ docs, doc }) {
  // Nothing to document is not an error worth showing - the docs are a helper,
  // and the settings around them must keep working regardless.
  if (!doc) return null;

  const contentType = doc.sample.headers['Content-Type'];
  const headerLines = Object.entries(doc.sample.headers)
    .map(([key, value]) => `${key}: ${value}`)
    .join('\n');

  return (
    <div className={styles.webhookDocs}>
      <p className={styles.webhookDocsHint}>{doc.hint}</p>

      <div className={styles.webhookDocsLabel}>Works with</div>
      <p className={styles.webhookDocsHint}>{doc.targets}</p>

      <div className={styles.webhookDocsLabel}>URL looks like</div>
      <pre className={styles.webhookDocsCode}><code>{doc.urlHint}</code></pre>

      <div className={styles.webhookDocsLabel}>ErgDash sends</div>
      <pre className={styles.webhookDocsCode}>
        <code>{`POST <your webhook URL>\n${headerLines}\n\n${formatSampleBody(doc.sample.body, contentType)}`}</code>
      </pre>
      ...
      <p className={styles.webhookDocsHint}>
        {docs?.app_origin
          ? `Links are absolute, built from APP_ORIGIN (${docs.app_origin}).`
...
function NotificationsSection() {
  ...
  const activeWebhookDoc = webhookDocs?.formats?.find(entry => entry.id === webhookFormat) ?? null;
          ...
          <>
            ...
            <div className={styles.row}>
              ...
              <input
                ...
                placeholder={activeWebhookDoc?.urlHint || 'https://ntfy.sh/my-topic'}
                value={webhookUrl}
            ...
            <WebhookDocs docs={webhookDocs} doc={activeWebhookDoc} />
          </>
        )}
        <div className={styles.row}>
          <div>

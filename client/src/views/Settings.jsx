...
import { WEBHOOK_FORMATS, webhookFormat as webhookFormat_ } from '../utils/webhookFormats.js';
import {
...
// Pretty-print the sample body so a JSON payload reads as JSON, while ntfy's
// plain-text body is shown as-is.
function formatSampleBody(body, contentType) {
...
// In-page documentation for the selected webhook format. The examples come
// from the server, generated by the same code that composes real deliveries,
// so what is documented here is exactly what gets sent.
// `format` is local UI copy and always present. `serverDocs` carries the
// byte-exact example and is optional: when the server cannot supply it, the
// guidance still renders rather than the whole panel vanishing.
function WebhookDocs({ format, serverDocs }) {
  const sampleDoc = serverDocs?.formats?.find(entry => entry.id === format.id) ?? null;

  return (
    <div className={styles.webhookDocs}>
      <p className={styles.webhookDocsHint}>{format.hint}</p>

      <div className={styles.webhookDocsLabel}>Works with</div>
      <p className={styles.webhookDocsHint}>{format.targets}</p>

      <div className={styles.webhookDocsLabel}>URL looks like</div>
      <pre className={styles.webhookDocsCode}><code>{format.urlHint}</code></pre>

      {sampleDoc && (
        <>
          <div className={styles.webhookDocsLabel}>ErgDash sends</div>
          <pre className={styles.webhookDocsCode}>
            <code>
              {`POST <your webhook URL>\n${
                Object.entries(sampleDoc.sample.headers).map(([k, v]) => `${k}: ${v}`).join('\n')
              }\n\n${formatSampleBody(sampleDoc.sample.body, sampleDoc.sample.headers['Content-Type'])}`}
            </code>
          </pre>
          {/* Whether links are absolute depends on the instance, so say which
              case the reader is actually looking at. */}
          <p className={styles.webhookDocsHint}>
            {serverDocs.app_origin
              ? `Links are absolute, built from APP_ORIGIN (${serverDocs.app_origin}).`
              : 'Set APP_ORIGIN on the server to include a clickable link back to ErgDash.'}
          </p>
        </>
      )}
    </div>
...
function NotificationsSection() {
  ...
  // Server-generated webhook documentation, fetched once. Held here rather
  // than inside WebhookDocs so the URL placeholder can follow the format too.
  const [webhookDocs, setWebhookDocs] = useState(null);
  ...
  useEffect(() => {
    ...
    setWebhookFormat(settings.notify_webhook_format ?? 'json');
  }, [settings]);
  ...
  const activeFormat = webhookFormat_(webhookFormat);
        ...
        {channels.includes('webhook') && (
          <>
            <SelectRow
              ...
              subtext="Payload shape for your target service — there is no universal one"
              value={webhookFormat}
              onChange={value => {
                setWebhookFormat(value);
                save({ notify_webhook_format: value });
              }}
              options={WEBHOOK_FORMATS.map(f => ({ value: f.id, label: f.label }))}
            />
            <div className={styles.row}>
              <div>
                <div className={styles.label}>Webhook URL</div>
                <div className={styles.subtext}>
                  Where each notification is POSTed
                </div>
              </div>
              <input
                type="url"
                className={styles.fileInput}
                placeholder={activeFormat.urlHint}
                value={webhookUrl}
            ...
            <WebhookDocs format={activeFormat} serverDocs={webhookDocs} />
          </>
        )}
        <div className={styles.row}>
          <div>

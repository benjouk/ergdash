...
- **Progress:** Fitness (CTL/ATL/TSB), pace trend, power-duration curve with 90-day ghost, time-in-zone and polarization stacks, efficiency (watts/beat), distance per stroke, stroke quality, HR drift, cumulative metres race line, drag factor timeline, PB progression, fade fingerprint, organized into Training Load / Speed & Racing / Technique & Efficiency / Plan sections with an editable layout
- **Tools:** Pace / watts / cal-per-hour converter, a Concept2 weight-adjusted score calculator, a race plan builder with even, negative, and aggressive pacing strategies, and a predicted-times table covering every benchmark distance - trend projections where you have recent results, pace-per-doubling estimates (fitted to your own results, falling back to Paul's Law) where you don't

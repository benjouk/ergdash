...
function addWorkout(date) {
  db.prepare(`
    ...
    VALUES (?, 1, 1, ?, 'rower', 'FixedDistanceSplits', 5000, 1200000, 120000, datetime('now'))
  `).run(nextId++, date);
}

describe('computeWeekStreak', () => {
  it('returns 0 with no workouts', () => {
    expect(computeWeekStreak(db, 1)).toBe(0);
  });
  ...
  it('counts consecutive weeks', () => {
    ...
    expect(computeWeekStreak(db, 1)).toBe(3);
  });
  ...
  it('breaks on a missed week', () => {
    ...
    expect(computeWeekStreak(db, 1)).toBe(1);
  });
